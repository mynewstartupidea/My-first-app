...
import { UserCheck, Mail, Shield, Crown, Users, Plus, Trash2 } from 'lucide-react'
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {[
          ...
          <div key={r.role} className="bg-white rounded-2xl border border-slate-100 p-4">
            <div className={`w-7 h-7 rounded-lg flex items-center justify-center mb-2 ${r.cls.split(' ')[1]}`}>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        {!members || members.length === 0 ? (
          ...
          <div className="divide-y divide-slate-50">
            {/* Header */}
            <div className="hidden lg:grid grid-cols-[1fr_180px_100px_100px_40px] gap-4 px-5 py-2 text-[10px] font-semibold text-slate-400 uppercase tracking-wide bg-slate-50/50">
              <div>Member</div><div>Invited</div><div>Role</div><div>Status</div><div />
            </div>
            {members.map(m => (
              <div key={m.id} className="flex lg:grid lg:grid-cols-[1fr_180px_100px_100px_40px] items-center gap-4 px-5 py-4 hover:bg-slate-50/70 transition">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    {m.joined_at && <p className="text-xs text-slate-400">Joined {timeAgo(m.joined_at)}</p>}
                  </div>
                </div>
                ...
                <div className="hidden lg:flex items-center gap-1">
                  {roleIcon[m.role]}
                  <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full capitalize ${roleColors[m.role] ?? roleColors.member}`}>
                    {m.role}
                  </span>
                </div>
                <div className="hidden lg:block">
                  <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full capitalize ${m.status === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                    {m.status}
                  </span>
                </div>
                <form action="/api/team/members" method="POST" className="hidden lg:block">
